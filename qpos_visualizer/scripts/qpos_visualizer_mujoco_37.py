...
class QposVisualizerMuJoCo:
    def __init__(self, model_path="src/legged_mppi/scripts/models/go1/go1_scene_mppi_pyr.xml", replay=False,
                 save_frames=False, frame_rate=50, save_dir='src/legged_mppi/frames'):
        ...
        #self.agent = MPPI(task='med_box')
        #self.agent = MPPI(task='med_2_box')
        #self.agent = MPPI(task='stand_hw')

        # self.goal_index = 0
        # self.goals = [[-2.9, 0.01439, 0.27],
        #               [-2.124, 0.01439, 0.27],
        #               #[-1.67, 0.01, 0.55],
        #               [-1.304, 0.03139, 0.51], #1
        #               [-0.304, 0.03139, 0.27],
        #               [-0.304, 0.03139, 0.27]]
        ...
        self.state = np.zeros(37)  # nq is the number of generalized coordinates (joints)
    ...
    def replay_from_bag(self, bag_file):
        ...
        for topic, msg, t in bag.read_messages():
            if topic == "/mocap_node/Go1_body/Odom":
    ...
    def mocap_robot_callback(self, data):
        ...
        # print("odom_callback")
        self.state[:3] = [data.pose.pose.position.x, data.pose.pose.position.y, data.pose.pose.position.z+0.02]
    ...
    def run(self):
        ...
        # self.agent.body_ref[:3] = self.agent.goal_pos[self.agent.goal_index]
        
        while not rospy.is_shutdown():
            self.state[7:10] = [self.joint_states["FR_hip"].q, self.joint_states["FR_thigh"].q, self.joint_states["FR_calf"].q]
            self.state[10:13] = [self.joint_states["FL_hip"].q, self.joint_states["FL_thigh"].q, self.joint_states["FL_calf"].q]
            self.state[13:16] = [self.joint_states["RR_hip"].q, self.joint_states["RR_thigh"].q, self.joint_states["RR_calf"].q]
            self.state[16:19] = [self.joint_states["RL_hip"].q, self.joint_states["RL_thigh"].q, self.joint_states["RL_calf"].q]
            self.state[19:22] = [0,0,0]
            self.state[22:25] = [0,0,0]
            self.state[25:28] = [self.joint_states["FR_hip"].dq, self.joint_states["FR_thigh"].dq, self.joint_states["FR_calf"].dq]
            self.state[28:31] = [self.joint_states["FL_hip"].dq, self.joint_states["FL_thigh"].dq, self.joint_states["FL_calf"].dq]
            self.state[31:34] = [self.joint_states["RR_hip"].dq, self.joint_states["RR_thigh"].dq, self.joint_states["RR_calf"].dq]
            self.state[34:37] = [self.joint_states["RL_hip"].dq, self.joint_states["RL_thigh"].dq, self.joint_states["RL_calf"].dq]

            self.data.qpos[:] = self.state[:19] 
            self.data.qvel[:] = self.state[19:37] 

            # Step the MuJoCo simulation forward
            mujoco.mj_step(self.model, self.data)

            # if np.linalg.norm(self.state[:3] - np.array(self.goal)) < 0.2 and self.goal_index < len(self.goals)-1 :
            #     self.goal_index += 1
            #     self.goal = self.goals[self.goal_index]
            
            # self.viewer.add_marker(
            #         pos=self.agent.goal_pos[self.agent.goal_index],         # Position of the marker
            ...
            # error = np.linalg.norm(np.array(self.agent.body_ref[:3]) - np.array(self.state[:3]))
            # print(error)
            # if error < 0.2:
...
if __name__ == '__main__':
    try:
        ...
        visualizer.set_camera_parameters(azimuth=-90, elevation=-45, distance=2.7, lookat=[-1.304, 0.03139, 0.15])
